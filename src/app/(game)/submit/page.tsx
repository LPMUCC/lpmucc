...
export default function Submit() {
  ...
  const handleSubmit = async () => {
    if (!keyNumber || !keyWord || !chapter || !file) {
      setError('// all fields required. the vault accepts no incomplete transmissions.')
      return
    }
    setLoading(true)
    setError('')
    try {
      ...
      const { error: uploadError } = await supabase.storage
        .from('vault-artifacts').upload(path, file, { upsert: false })
      if (uploadError) throw uploadError

      // Submit via API (server sets timestamp)
  ...
  if (success) return (
    <div className="min-h-screen flex items-center justify-center px-8" style={{ background: '#04342C' }}>
      <div className="font-mono text-center max-w-md">
        ...
        <div className="text-xs mb-2" style={{ color: '#0F6E56' }}>Response time: variable.</div>
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center px-8"
      ...
      <div className="w-full max-w-lg font-mono">
        <div className="text-xs tracking-widest mb-8" style={{ color: '#0F6E56' }}>
          // SUBMIT A VAULT KEY
        </div>

        {[
          { label: '// WHICH KEY ARE YOU SUBMITTING? (1-33)', value: keyNumber,
            set: setKeyNumber, type: 'number', max: '33' },
          { label: '// ENTER THE KEY WORD', value: keyWord,
            set: (v: string) => setKeyWord(v.toUpperCase()), type: 'text' },
          { label: '// WHICH CHAPTER CONTAINS THIS KEY?', value: chapter,
            set: setChapter, type: 'text' },
        ].map(({ label, value, set, type, max }) => (
        ...
        <div className="mb-8">
          ...
          <div className="text-xs mb-3" style={{ color: '#085041' }}>
            The artifact must demonstrate that you did the work described in the chapter.
            Not that you read it. That you did it.
          </div>
          <input
            type="file"
            accept="image/*,.pdf"
