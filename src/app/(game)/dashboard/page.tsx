...
const VAULT_KEYS: Record<number,string> = {
  ...
  10:'VOLUNTARY',11:'SOURCE',12:'WITHDRAW',13:'TENDER',14:'REBUT',15:'CORRECT',16:'BURDEN',17:'RECORD',
  18:'OFFICE',19:'ISSUE',20:'INSTRUMENT',21:'NETWORK',22:'DYNASTY',23:'ROCKEFELLER',24:'PATENT',
  25:'ALLODIAL',26:'TRUST',27:'PROTECTOR',28:'CENTURY',29:'BANKER',30:'PROBATE',31:'PRIVATE',32:'TEACH',33:'LEGACY'
}
    ...
    <div style={{ background: '#04342C', minHeight: '100vh', fontFamily: 'Courier New, monospace' }}>
      <div style={{ display:'flex', justifyContent:'space-between', padding:'16px 32px', borderBottom:'1px solid #0F6E56' }}>
        ...
        <button onClick={() => supabase.auth.signOut().then(() => router.push('/'))}
          style={{ background:'transparent', border:'none', color:'#0F6E56', fontFamily:'Courier New, monospace', fontSize:'11px', cursor:'pointer' }}>
          // sign out
        </button>
      </div>
      ...
      <div style={{ maxWidth:'720px', margin:'0 auto', padding:'48px 32px' }}>
        ...
        <div style={{ marginTop:'32px', display:'flex', flexDirection:'column', gap:'12px' }}>
          <button onClick={() => router.push('/submit')}
            style={{ background:'transparent', border:'1px solid #1D9E75', color:'#1D9E75', fontFamily:'Courier New, monospace', fontSize:'13px', padding:'12px 16px', cursor:'pointer', textAlign:'left' }}>
            // SUBMIT A KEY →
          </button>
          {keysVerified.length >= 9 && !profile?.book_2_activated_at && (
            <button onClick={() => router.push('/activate?book=2')}
              style={{ background:'transparent', border:'1px solid #EF9F27', color:'#EF9F27', fontFamily:'Courier New, monospace', fontSize:'13px', padding:'12px 16px', cursor:'pointer', textAlign:'left' }}>
              // ACTIVATE BOOK TWO →
            </button>
          )}
          {keysVerified.length >= 21 && !profile?.book_3_activated_at && (
            <button onClick={() => router.push('/activate?book=3')}
              style={{ background:'transparent', border:'1px solid #7F77DD', color:'#7F77DD', fontFamily:'Courier New, monospace', fontSize:'13px', padding:'12px 16px', cursor:'pointer', textAlign:'left' }}>
              // ACTIVATE BOOK THREE →
            </button>
          )}
          {keysVerified.length === 33 && (
            <button onClick={() => router.push('/vault')}
              style={{ background:'transparent', border:'1px solid #BA7517', color:'#BA7517', fontFamily:'Courier New, monospace', fontSize:'13px', padding:'12px 16px', cursor:'pointer', textAlign:'left' }}>
              // THE VAULT →
            </button>
          )}
        </div>
